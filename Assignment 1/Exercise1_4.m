...
t = linspace(0, T, n);
for i = 2:n
    u(i) = (((t(i)^3 - t(i-1)^3)/3 + (u(i-1)*(1 - 2*h)))/(1 + 2*h));
end
...
syms t
hold on
fplot(31/32*exp(-4*t) + t^2/4 - t/8 + 1/32, [0 T], 'LineStyle', '-')
plot(ts, u, 'LineStyle', '--')
...
%% c) Plot of the solution and some schemes. 
ts = linspace(0, T, N);
syms t
hold on
fplot(exp(-a*t), [0 T], 'LineStyle', '-')
plot(ts, uExp, 'LineStyle', '--')
plot(ts, uImp, 'Color', '#77AC30', 'LineStyle', ':')
plot(ts, uCN, 'LineStyle', '-.')
title("Solutions with a = " + a + ", k = " + k + ".")
